...
def test_high_issue_subtracts_20():
    issues = [Issue(id="a", severity="high", location="/", description="x")]
    assert calculate_score(issues) == 80


def test_medium_issue_subtracts_12():
    issues = [Issue(id="a", severity="medium", location="/", description="x")]
    assert calculate_score(issues) == 88


def test_low_issue_subtracts_6():
    issues = [Issue(id="a", severity="low", location="/", description="x")]
    assert calculate_score(issues) == 94


def test_multiple_issues_subtract_correctly():
    issues = [
        Issue(id="a", severity="high", location="/", description="x"),
        Issue(id="b", severity="medium", location="/", description="x"),
    ]
...
def test_score_never_goes_below_0():
    issues = [
        Issue(id=f"a{i}", severity="high", location="/", description="x")
        for i in range(10)
    ]
